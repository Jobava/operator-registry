...
func newRegistryServeCmd() *cobra.Command {
	...
	rootCmd.Flags().StringP("timeout-seconds", "s", "infinite", "Timeout in seconds, default is infinite")
	return rootCmd
...
func serveFunc(cmd *cobra.Command, args []string) error {
	...
	s := grpc.NewServer()
	if timeout != "infinite" {
		timeoutSeconds, err := strconv.ParseInt(timeout, 10, 16)
		if err != nil {
			return err
		}

		timeoutDuration := time.Duration(timeoutSeconds) * time.Second
		s = grpc.NewServer(grpc.ConnectionTimeout(timeoutDuration))
	}
